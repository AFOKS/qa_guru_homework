...
#  Запустить все тесты без использования PageFactory и с использованием, сравнить количество пройденных и падений, сравнить ошибки при падениях
#  Если количество падений разное - найти ответ почему? ну или задать вопрос сперва Алексу, а потом и лектору :)
...
#  как улучшить проверки в существующих тестах с предлагаемыми данными?
...
def test_positive_form_submission(driver, name, email, cur_addr, perm_addr):
    #page = TextBoxPage(driver).open() #  Мы еще поговорим о таком варианте реализации
    ...
    # Мы еще поговорим о таком варианте реализации
...
@pytest.mark.parametrize("invalid_email", [
    ...
    "john.doe@com",          # Нет доменной зоны верхнего уровня -  обратите особое внимание
    "john@missing-dot",      # Нет точки в домене -обратите особое внимание
...
def test_long_input_fields(driver, field_type, long_string):
    ...
    #  условные операторы в тесте, как можно от них избавиться и станет ли от этого понятнее решение?
...
@pytest.mark.parametrize("security_payload", [
    "<script>alert('xss')</script>",                 # Базовый XSS скрипт -  нашли и исправили ошибку?
    "1' OR '1'='1",                                  # Базовая SQL-инъекция
    ":):):):))))::;)",                               # Суррогатные пары (Эмодзи)
    "<div>HTML injection</div>"                      # Теги верстки - нашли ошибку? -  нашли и исправили ошибку?
])
